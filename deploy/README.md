...
6. `cd deploy && docker compose -f docker-compose.silkroad.yml up -d --build` — API, brain worker, Photon iMessage connector, Gmail connector, Mongo.
...
8. iMessage (the agent's own number): create a project at https://app.photon.codes, put `PHOTON_PROJECT_ID` / `PHOTON_PROJECT_SECRET` in `.env`, register the owner's phone under Users and set `PHOTON_OWNER_HANDLE` to it, then `npm run photon:hello` (texts the owner the contact card from the agent's number). The `photon` service answers from then on. Optional, Mac only: `npm run channel:imessage` on the client's Mac ingests their other iMessage threads (copy `SILKROAD_SERVICE_TOKEN`, set `SILKROAD_API_URL=https://<vps-host>` and `IMESSAGE_IGNORE_CHATS=<photon line>`, Full Disk Access + Messages signed in). Without Docker, `npx pm2 start config/ecosystem.config.js` supervises all processes on any host.
